...
hold on
p2=semilogy(SNR,MSE_block_v_SD_001_M_2(1,1:21),'-');
hold on
p3=semilogy(SNR,MSE_block_v_SD_001_M_2(2,1:21),':.');
hold on
p4=semilogy(SNR,MSE_delta_LR(1,:),'-.');
hold on
p5=semilogy(SNR,MSE_delta_fitz(1,:),'--');
grid on
p6=semilogy(SNR,MSE_delta_NM_M_2(1,:),'--.');
hold on
p7=semilogy(SNR,MSE_delta_NM_M_2(2,:),'--.');
hold on
p8=semilogy(SNR,MSE_delta_kay(1,:),'.-');
hold on
[hc,ht,hcl] = nice_plot(gcf); 
lgnd=legend({'CRVB','SL ($v{=}1$)','SL ($v{=}32$)','L\&R','Fitz','NM ($v{=}1$ SL based)','NM ($v{=}32$ SL based)','Kay'},'Interpreter','latex','Location','southwest');
