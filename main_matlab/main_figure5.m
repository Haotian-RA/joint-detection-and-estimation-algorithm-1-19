...
M=1;LL=100; 
n_L_0=0;  
L=8;beta=0.5;c_init=10;v_set=[1 16 32];
...
%     [tx,symbol] = Gold_sequence(L_0,c_init,M,L,beta);
    
    [tx,symbol] = Gold_sequence(L_0,c_init);

    N=length(tx);
    tx_shift_register_AK = tx_register_AK(tx);
    sig = sig_generator(tx,normalized_true_freq_offset,true_phase_offset);
    down_sig = sig_generator(symbol,normalized_true_freq_offset,true_phase_offset);
    symbol=tx(1:M:end);
    ...
    for snr=SNR
        ...
        for count=1:LL
            ...
            down_rx = rx_generator(down_sig,snr);
            ...
            z=down_rx.*conj(symbol); % unit energy is decreased by M^2 
...
legend('CRVB','1-NM','16-NM','32-NM','kay','LR','fitz')
ylabel('MSE(M\delta)')
xlabel('E_s/N_0(dB)')
