...
The SL estimator with $v=1$ has slightly better accuracy 
at 10 dB SNR than with $v=32$.
The gap is due to the Dirichlet function for
$\delta \neq 0$.
...
For example,
at $\SI{-2}{\dB}$ SNR, $\gamma=0.44$, the false alarm probability $P_{FA}$ of SL with $v=32$ is reduced $0.2\%$ and 
the detection probability $P_{D}$ is $5\%$ larger compared with the SL with $v{=}1$. 
The figure also shows that the detector does not work well at $\SI{-4}{\dB}$ SNR if only $32$ symbols of preamble are used;  
